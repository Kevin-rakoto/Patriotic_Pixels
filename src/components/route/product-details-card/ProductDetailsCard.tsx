import { Dispatch, FC, SetStateAction, useState } from "react"
import { IProductData } from "../../../static/types/productDataType"
import { RxCross1 } from "react-icons/rx";
import { Link } from "react-router-dom";
import { AiFillHeart, AiOutlineHeart, AiOutlineMessage, AiOutlineShoppingCart } from "react-icons/ai";
import styles from "../../../styles/styles";

type ProductDetailsCardProps = {
    setOpen: Dispatch<SetStateAction<boolean>>,
    data?: IProductData,
}

const ProductDetailsCard:FC<ProductDetailsCardProps> = ({data, setOpen}) => {
  const [count, setCount] = useState<number>(1);
  const [click, setClick] = useState<boolean>(false);
  const [select, setSelect] = useState<boolean>(false);

  const handleMessageSubmit = () => {

  }

  const decrementCount = () => {
    if (count > 1) {
      setCount(count - 1);
    }
  };

  const incrementCount = () => {
    setCount(count + 1);
  };

  return (    
    <div className="bg-[#fff]">
      {data ? (
        <div className="fixed w-full h-screen top-0 left-0 bg-[#00000030] z-40 flex items-center justify-center">
          <div className="w-[90%] 800px:w-[60%] h-[90vh] overflow-y-scroll 800px:h-[75vh] bg-white rounded-md shadow-sm relative p-4">
            <RxCross1
              size={30}
              className="absolute right-3 top-3 z-50"
              onClick={() => setOpen(false)}
            />

            <div className="block w-full 800px:flex">
              <div className="w-full 800px:w-[50%]">
                <img src={`${data.image_Url && data.image_Url[0]?.url}`} alt="" />
                <div className="flex">
                  <Link to={`/shop/preview/${data.shop.shop_avatar.url}`} className="flex">
                    <img
                      src={`${data.image_Url && data.image_Url[0]?.url}`}
                      alt=""
                      className="w-[50px] h-[50px] rounded-full mr-2"
                    />
                    <div>
                      <h3 className={`${styles.shop_name}`}>
                        {data.shop.name}
                      </h3>
                      <h5 className="pb-3 text-[15px]">{data?.rating} Ratings</h5>
                    </div>
                  </Link>
                </div>
                <div
                  className={`${styles.button} bg-[#000] mt-4 rounded-[4px] h-11`}
                  onClick={handleMessageSubmit}
                >
                  <span className="text-[#fff] flex items-center">
                    Comment <AiOutlineMessage className="ml-1" />
                  </span>
                </div>
                <h5 className="text-[16px] text-[red] mt-5"></h5>
              </div>

              <div className="w-full 800px:w-[50%] pt-5 pl-[5px] pr-[5px]">
                <h1 className={`${styles.productTitle} text-[20px]`}>
                  {data.name}
                </h1>
                <p>{data.description}</p>

                <div className="flex pt-3">
                  <h4 className={`${styles.productDiscountPrice}`}>
                  </h4>
                  <h3 className={`${styles.price}`}>
                  </h3>
                </div>
                <div
                  className={`${styles.button} mt-6 rounded-[4px] h-11 flex items-center`}
                  onClick={() => console.log("addToCartHandler(data._id)")}
                >
                  <span className="text-[#fff] flex items-center">
                  </span>
                </div>
              </div>
            </div>
          </div>
        </div>
      ) : null}
    </div>
  )
}

export default ProductDetailsCard